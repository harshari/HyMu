...
def visualize_bounding_box(exp_dir, surface_floorplan, embedded_floorplan, surface_clusters, embedded_clusters, title, E_s):
    """
    Visualize bounding boxes for surface and embedded tiers side by side, including edge spacing.
    ...
    def plot_floorplan(ax, floorplan, tier_title):
        ...
        # Compute bounding box **including** edge spacing (E_s)
        ...
        # Draw bounding box with E_s expansion
        ax.plot(
            [-E_s, max_y, max_y, -E_s, -E_s],  # Extend edges by E_s
            [-E_s, -E_s, max_x, max_x, -E_s],
            color="black",
            linestyle="--",
            linewidth=2,
...
for i, (tier1, tier2) in enumerate(zip(tier1_permutations, tier2_permutations)):
    ...
    combined_floorplan = floorplan_data_surface + floorplan_data_embedded
    ...
    if combined_floorplan_hash in seen_floorplans:
        print(f"Skipping duplicate initial placement at permutation {i + 1}")
        continue
    else:
        seen_floorplans.add(combined_floorplan_hash)
    ...
    # Ensure unique filenames by appending perturbation number
    starting_point_id = f"starting_point_1_perturb_{perturbation_id}" if perturbation_id > 0 else "starting_point_1"

    visualize_chiplet_floorplan(f"{exp_dir}/{starting_point_id}_surface", adjusted_surface, clusters_surface, 0.5, "Tier 1: Surface Floorplan")
    visualize_chiplet_floorplan(f"{exp_dir}/{starting_point_id}_embedded", adjusted_embedded, clusters_embedded, 0.5, "Tier 0: Embedded Floorplan")
    ...
    visualize_bounding_box(
        ...
        E_s = .5  # Adjust edge spacing value as needed
    )
